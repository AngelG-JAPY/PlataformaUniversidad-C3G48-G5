<template>
  <v-contaier>
    <span class="fondo"></span>
    <v-row aling="center" justify="center">
      <v-col cols="12" sm="10">
        <v-card class="elevation-6 mt-8" color="white">
          <div class="text-end">
            <v-btn class="ma-2 text-end" outlined large fab color="indigo">
              <v-icon>mdi-pencil</v-icon>
            </v-btn>
          </div>
          <v-row no-gutters>
            <v-col cols="12" xs="12" sm="6" md="6">
              <v-card-text class="mt-12">
                <h2 class="text-center">Foto</h2>
                <div class="mt-5">
                <v-img 
                  src="data:image/png;base64,"
                  height="250"
                  contain
                >
                </v-img>
                </div>
              </v-card-text>
            </v-col>

            <v-col cols="12" xs="12" sm="12" md="6">
              <v-card-text class="mt-12">
                <h2 class="text-center">Tu Informacion</h2>

                <v-text-field
                  label="Nombre:"
                  outlined
                  dense
                  color="blue"
                  autocomplete="false"
                  class="mt-6"
                />
                <v-text-field
                  label="Apellido"
                  outlined
                  dense
                  color="blue"
                  autocomplete="false"
                  class="mt-2"
                />

                <v-text-field
                  label="Correo:"
                  outlined
                  dense
                  color="blue"
                  autocomplete="false"
                  class="mt-2"
                />
              </v-card-text>
              <div class="ml-4 mr-4" cols="4">
                <v-btn color="black" dark block>Cambio De Contraseña</v-btn>
              </div>
            </v-col>
          </v-row>
          <v-row justify="center">
            <v-col cols="12" xs="12" md="5">
              <div class="ml-4 mr-4">
                <v-select :items="items" label="Tus Favoritos" outlined></v-select>
              </div>
            </v-col>
            <v-col cols="12" xs="6" md="5">
              <div class="ml-1 mr-4">
                <v-select :items="items" label="" outlined></v-select>
              </div>
            </v-col>
          </v-row>
          <v-row class="mb-5" justify="center">
            <v-col cols="6">
              <v-btn color="blue" dark block tile>GUARDAR</v-btn>
            </v-col>
          </v-row>
        </v-card>
      </v-col>
    </v-row>
  </v-contaier>
</template>


<script>
export default {};
</script>
<style scoped>
.fondo {
  width: 100%;
  height: 100%;
  position: absolute;
  top: 0;
  left: 0;

  background: url("https://media.istockphoto.com/photos/blue-background-picture-id996078816?k=20&m=996078816&s=612x612&w=0&h=RnDOSWqpE3SghSazzNztGF-w9pDg0PiQKuRRbLJcyEY=")
    no-repeat center center;
  background-size: cover;
  background-color: grey;
  background-attachment: scroll;
}
</style>c