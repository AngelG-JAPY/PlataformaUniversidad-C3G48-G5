<template>
  <v-app >
    <v-parallax
      style="width:100%;"
      dark
      src="https://png.pngtree.com/thumb_back/fw800/background/20190222/ourmid/pngtree-news-interview-camera-microphone-cartoon-background-image_58927.jpg"
    >
      <v-row align="center" justify="center">
        <v-col class="text-center" cols="12">
          <h1 class="text-h4 font-weight-thin mb-4">Noticias</h1>
          <h4 class="subheading">¡¡Enterate de las novedades de cada universidad!! </h4>
        </v-col>
      </v-row>
    </v-parallax>
    

    <v-row>
      <v-col cols="12" sm="12" lg="6" md="4" class="ml-10">
        <v-radio-group v-model="row" row>
          <v-radio label="Todas" value="radio-1"></v-radio>
          <v-radio label="Solo favoritas" value="radio-2"></v-radio>
        </v-radio-group>
      </v-col>
    </v-row>
    

    <v-row justify="start">
      <v-col cols="12" sm="12" lg="4">
        <v-card class="mx-auto" max-width="344">
          <v-img
            src="data:image/jpeg;base64,"
            height="200px"
          ></v-img>

          <v-card-title> GRAN EVENTO ESTE VIERNES </v-card-title>

          <v-card-subtitle> Mas de mil personas asistiran </v-card-subtitle>

          <v-card-actions>
            <v-btn
              color="blue"
              href="https://www.youtube.com/channel/UC_Hb-oO8XlU0xh-ry0idI1A/featured"
              text
            >
              Leer Mas
            </v-btn>

            <v-spacer></v-spacer>

            <v-btn icon @click="show = !show">
              <v-icon>{{
                show ? "mdi-chevron-up" : "mdi-chevron-down"
              }}</v-icon>
            </v-btn>
          </v-card-actions>

          <v-expand-transition>
            <div v-show="show">
              <v-divider></v-divider>

              <v-card-text>
                I'm a thing. But, like most politicians, he promised more than
                he could deliver. You won't have time for sleeping, soldier, not
                with all the bed making you'll be doing. Then we'll go with that
                data file! Hey, you add a one and two zeros to that or we walk!
                You're going to do his laundry? I've got to find a way to
                escape.
              </v-card-text>
            </div>
          </v-expand-transition>
        </v-card>
      </v-col>
      <v-col cols="12" sm="12" lg="4">
        <v-card class="mx-auto" max-width="344">
          <v-img
            src="https://conflictos-ambientales.net/oca_bd/img/Logo%20UN.jpg"
            height="200px"
          ></v-img>

          <v-card-title> Se abren inscripciones </v-card-title>

          <v-card-subtitle> inscripciones abiertas hasta el 24 de octubre </v-card-subtitle>

          <v-card-actions>
            <v-btn
              color="blue"
              href="https://www.youtube.com/channel/UC_Hb-oO8XlU0xh-ry0idI1A/featured"
              text
            >
              Leer Mas
            </v-btn>

            <v-spacer></v-spacer>

            <v-btn icon @click="show1 = !show1">
              <v-icon>{{
                show1 ? "mdi-chevron-up" : "mdi-chevron-down"
              }}</v-icon>
            </v-btn>
          </v-card-actions>

          <v-expand-transition>
            <div v-show="show1">
              <v-divider></v-divider>

              <v-card-text>
                I'm a thing. But, like most politicians, he promised more than
                he could deliver. You won't have time for sleeping, soldier, not
                with all the bed making you'll be doing. Then we'll go with that
                data file! Hey, you add a one and two zeros to that or we walk!
                You're going to do his laundry? I've got to find a way to
                escape.
              </v-card-text>
            </div>
          </v-expand-transition>
        </v-card>
      </v-col>
      <v-col cols="12" sm="12" lg="4">
        <v-card class="mx-auto" max-width="344">
          <v-img
            src="https://www.uao.edu.co/wp-content/uploads/2020/05/Vive-la-UAO-745x410.jpg"
            height="200px"
          ></v-img>

          <v-card-title> Evento cultural en la autonoma </v-card-title>

          <v-card-subtitle> La universidad Autonoma de occidente... </v-card-subtitle>

          <v-card-actions>
            <v-btn
              color="blue"
              href="https://www.youtube.com/channel/UC_Hb-oO8XlU0xh-ry0idI1A/featured"
              text
            >
              Leer Mas
            </v-btn>

            <v-spacer></v-spacer>

            <v-btn icon @click="show2 = !show2">
              <v-icon>{{
                show2 ? "mdi-chevron-up" : "mdi-chevron-down"
              }}</v-icon>
            </v-btn>
          </v-card-actions>

          <v-expand-transition>
            <div v-show="show2">
              <v-divider></v-divider>

              <v-card-text>
                I'm a thing. But, like most politicians, he promised more than
                he could deliver. You won't have time for sleeping, soldier, not
                with all the bed making you'll be doing. Then we'll go with that
                data file! Hey, you add a one and two zeros to that or we walk!
                You're going to do his laundry? I've got to find a way to
                escape.
              </v-card-text>
            </div>
          </v-expand-transition>
        </v-card>
      </v-col>
      <v-col cols="12" sm="12" lg="4">
        <v-card class="mx-auto" max-width="344">
          <v-img
            src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSNNYGe_UgpJqQMLXB1U_aCbpBsHh2KhGxcHg&usqp=CAU"
            height="200px"
          ></v-img>

          <v-card-title> Facultad de medicina </v-card-title>

          <v-card-subtitle> La facultad de medicina de la universidad de antioquia... </v-card-subtitle>

          <v-card-actions>
            <v-btn
              color="blue"
              href="https://www.youtube.com/channel/UC_Hb-oO8XlU0xh-ry0idI1A/featured"
              text
            >
              Leer Mas
            </v-btn>

            <v-spacer></v-spacer>

            <v-btn icon @click="show3 = !show3">
              <v-icon>{{
                show3 ? "mdi-chevron-up" : "mdi-chevron-down"
              }}</v-icon>
            </v-btn>
          </v-card-actions>

          <v-expand-transition>
            <div v-show="show3">
              <v-divider></v-divider>

              <v-card-text>
                I'm a thing. But, like most politicians, he promised more than
                he could deliver. You won't have time for sleeping, soldier, not
                with all the bed making you'll be doing. Then we'll go with that
                data file! Hey, you add a one and two zeros to that or we walk!
                You're going to do his laundry? I've got to find a way to
                escape.
              </v-card-text>
            </div>
          </v-expand-transition>
        </v-card>
      </v-col>
    </v-row>
  </v-app>
</template>

<script>
export default {
  data: () => ({
    show: false,
    show1: false,
    show2: false,
    show3: false,
  }),
};
</script>

<style scoped>
</style>